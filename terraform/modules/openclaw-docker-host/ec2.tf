...
resource "aws_key_pair" "docker_host" {
  ...
  public_key = tls_private_key.docker_host[0].public_key_openssh
...
resource "local_sensitive_file" "private_key" {
  count           = var.create_ssh_key ? 1 : 0
  content         = tls_private_key.docker_host[0].private_key_openssh
...
resource "local_file" "public_key" {
  count           = var.create_ssh_key ? 1 : 0
  content         = tls_private_key.docker_host[0].public_key_openssh
...
resource "aws_instance" "docker_host" {
  ...
  key_name                    = var.create_ssh_key ? aws_key_pair.docker_host[0].key_name : var.existing_key_name
  ...
  # Root volume configuration (larger for Docker)
  root_block_device {
  ...
  user_data = templatefile("${path.module}/user_data/init.sh.tftpl", {
    ...
    auto_start             = var.auto_start_containers
    bot_names              = join(" ", [for bot in var.bots : bot.name])
  })
